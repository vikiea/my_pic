...
![memory alignment](https://gitee.com/vikieq/my_pic/raw/master/uPic/2021/10/15/memory_alignment.png)
...
`unsafe` 标准库提供了 `Alignof` 方法，可以返回一个类型的对齐值，也可以叫做**对齐系数或者对齐倍数**。例如：
...
答案：**顺序会对 struct 的大小产生影响**。每个字段按照自身的对齐倍数来确定在内存中的偏移量，字段排列顺序不同，上一个字段因偏移而浪费的大小也不同。
...
![memory alignment](../../../../../../Books/high-performance-go/charpter-2/hpg-struct-alignment/memory_alignment_order.png)
...
- [七天用Go从零实现系列](https://geektutu.com/post/qa-golang.html)